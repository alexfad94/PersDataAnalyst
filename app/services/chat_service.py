...
class ChatService:
    ...
    def _handle_local_prompt(
        ...
        if action == "help":
            ...
            return self._new_message(
                "assistant",
                "Для активного файла могу показать предпросмотр, собрать анализ, построить `histogram`, `bar`, `line` или `pie`, сделать DOCX-отчёт и сохранить краткую markdown-сводку. Для выбора колонок напишите, например: `Построй line chart x: date y: revenue`.",
            )
    ...
    def _detect_action(self, normalized: str) -> str:
        if any(token in normalized for token in ("граф", "chart", "plot", "hist", "bar", "line", "pie", "круг", "диаграм")):
            return "chart"
    ...
    def _detect_chart_type(self, normalized: str) -> str:
        ...
        if any(token in normalized for token in ("line", "лине")):
            return "line"
        if any(token in normalized for token in ("pie", "круг", "доле")):
            return "pie"
        return "bar"
