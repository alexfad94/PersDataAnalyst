...
class ChartService:
    ...
    def generate_chart(
        ...
        if chart_type not in {"line", "bar", "histogram", "pie"}:
            raise FileReadError("Поддерживаются только графики line, bar, histogram и pie.")
    ...
    def _generate_table_chart(
        ...
        if chart_type == "histogram":
            ...
            pie_colors = ["#114b5f", "#6c8b6b", "#d06b4e", "#8d3f28", "#26789b", "#5a9a70", "#c48a3a", "#3d5a5b"]
            if numeric_columns:
                ...
                grouped = (
                    ...
                    .assign(**{group_y: lambda frame: pd.to_numeric(frame[group_y], errors="coerce")})
                    .dropna(subset=[group_y])
                ...
                if grouped.empty:
                    raise FileReadError("Недостаточно положительных значений для pie графика.")
                axis.pie(
                    grouped.values,
                ...
                axis.set_title(f"Pie chart: {group_y} by {group_x}")
                description = f"Доли суммы «{group_y}» по категориям «{group_x}»."
            else:
                counts = dataframe[selected_x].astype(str).value_counts().head(8)
                if counts.empty:
                ...
                description = f"Доли категорий по колонке «{selected_x}»."
    ...
    def _generate_image_chart(self, stored_file: StoredFile, chart_type: str) -> dict[str, Any]:
        ...
        if chart_type == "histogram":
            ...
            if array.ndim == 2:
                labels = ["L"]
                values = [float(grayscale.mean())]
                colors = ["#114b5f"]
            else:
                labels = list(image.getbands())
                values = [float(array[:, :, index].mean()) for index in range(array.shape[2])]
                colors = ["#114b5f", "#6c8b6b", "#d06b4e", "#8d3f28"][: len(labels)]
            axis.pie(values, labels=labels, autopct="%1.1f%%", colors=colors, startangle=90)
